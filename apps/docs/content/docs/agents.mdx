...
type MockVisibility = Visibility & {
  setState(state: VisibilityState): void
}

declare function createMockVisibility(
  initialState?: VisibilityState,
): MockVisibility
declare function createDocumentVisibility(): Visibility | undefined
...
- [ ] With injectable visibility, hidden suspends schedules; visible catch-up notifies once and resumes.
...
Every Wave 1 adapter ships the instance-owning binding plus its option and return types, and also exports the thin `Store<T>` bridge (`useStore` / `toSvelteStore`) so a component can bind a stopwatch it does not own.
