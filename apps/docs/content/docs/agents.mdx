...
```ts
declare function useStore<T>(store: Store<T>): T // @watchstop/react
declare function useStore<T>(store: Store<T>): Readonly<ShallowRef<T>> // @watchstop/vue
declare function useStore<T>(store: Store<T>): Accessor<T> // @watchstop/solid
declare function toSvelteStore<T>(store: Store<T>): Readable<T> // @watchstop/svelte
```

Each adapter has exactly one export. React, Vue, and Solid keep the `useStore` name their ecosystems expect. Svelte uses `toSvelteStore` because `svelte/store` already exports `fromStore` / `toStore` for the opposite conversion.
