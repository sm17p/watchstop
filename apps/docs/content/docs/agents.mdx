...
Every Wave 1 adapter ships two exports: an **instance-owning** entry point for the common case, and the **primitive** bridge for instances the component does not own.
...
React, Vue, and Solid keep the `useStore` / `useStopwatch` names their ecosystems expect. Svelte uses `createStopwatch` and `toSvelteStore` because `svelte/store` already exports `fromStore` / `toStore` for the opposite conversion.
