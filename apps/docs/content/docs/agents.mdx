...
4. Export public API from package entry exactly as named below.
5. Typecheck with **TS7 (`tsgo`)**; `pnpm test` + `pnpm build` green.
...
declare function createMockClock(): MockClock
...
- [ ] `schedule` never runs the callback synchronously.
- [ ] `advance(ms)` runs due callbacks; nested `schedule` during flush waits for a later `advance`.
- [ ] `cancel` is idempotent and prevents the callback.
- [ ] `createTimerClock({ intervalMs })` uses that delay for `setTimeout`.
- [ ] `createBrowserClock` uses rAF + `performance.now` (skip or mock in Node unit tests; guard with feature detection).
...
- [ ] `start` → `advance` → `get()` reflects elapsed; subscribers receive updates.
...
- [ ] Built with tsdown; declarations emitted.
...
- Bind with `useSyncExternalStore(store.subscribe, store.get, store.get)`.
- Do not use `useState` + manual subscribe as the primary path.
- SSR: `get` must be safe without `window`.
...
Framework pages under `/docs/frameworks/*` stay stubs until the matching package ships (Step 7). Agents implementing adapters should still follow the constraints above.
