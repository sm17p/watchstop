...
Normative `StopwatchOptions` behavior is on [Options](/docs/core/options).
...
- [ ] Multiple running stopwatches on one clock identity produce one underlying `schedule` per tick wave.
- [ ] `detectClock()` / bare `new Stopwatch()` still allocate a fresh clock each call (sharing requires passing one clock).
...
Every adapter ships exactly **one** entry point: the instance-owning binding, plus its option and return types. A component either owns its stopwatch or does not use the adapter.
...
```ts
type UseStopwatchOptions = { clock?: Clock; precisionMs?: number; reactiveElapsed?: boolean }
...
```ts
type CreateStopwatchOptions = { clock?: Clock; precisionMs?: number; reactiveElapsed?: boolean }
...
- The options are `clock`, `precisionMs`, and `reactiveElapsed` (default `true`). `clock` and `precisionMs` are forwarded to `Stopwatch`. See [Options](/docs/core/options) for normative `StopwatchOptions` behavior.
- `reactiveElapsed: false` skips bridging tick notifies into framework `elapsed` while keeping `running` / start / stop / reset reactive. Snapshot `elapsed` on control transitions (`running` changed or not running). Live digits use `stopwatch.subscribe` → DOM — see [Vanilla](/docs/frameworks/vanilla).
...
```ts
type InjectStopwatchOptions = { clock?: Clock; precisionMs?: number; reactiveElapsed?: boolean }
...
- Subscribe **only on the client** via `useVisibleTask$`. No SSR subscription leaks. Sync `running` from `stopwatch.running` in that subscribe callback. When `reactiveElapsed` is false, skip writing the `elapsed` signal on ticks.
...
- `init` performs imperative `subscribe` (writes `elapsed` and `running` through `this` for Alpine reactivity). When `reactiveElapsed` is false, skip assigning `this.elapsed` on ticks.
