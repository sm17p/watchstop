...
```ts
type UseStopwatchOptions = { clock?: Clock }
...
```ts
type CreateStopwatchOptions = { clock?: Clock }
...
- The only option is `clock`. A `precision`-style option that coarsens notifications is out of scope for Wave 1 and will follow separately.
...
```ts
type InjectStopwatchOptions = { clock?: Clock }
...
- Bridge `Store` → Angular **signal** (writable internally, exposed via `asReadonly`).
...
- Subscribe **only on the client** via `useVisibleTask$`. No SSR subscription leaks.
...
- `init` performs imperative `subscribe` (writes `elapsed` through `this` for Alpine reactivity).
