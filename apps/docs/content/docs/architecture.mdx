...
Factories exist where the framework has no hook or inject context to hang ownership and teardown on. The binding shape is the same everywhere: `elapsed`, `start`, `stop`, `reset`, and the owned `stopwatch`.
