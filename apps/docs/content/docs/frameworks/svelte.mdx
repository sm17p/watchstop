...
| `reactiveElapsed` | `boolean` | Default `true`. When `false`, the elapsed `Readable` does not notify on ticks; `$running` and controls still update. Snapshot elapsed on control transitions. |
...
Pass `{ reactiveElapsed: false }` so the elapsed readable skips tick notifies while `$running` stays reactive. Avoid `$stopwatch` for live digits; write from `stopwatch.stopwatch.subscribe` into the DOM. See [Vanilla](/docs/frameworks/vanilla).
