...
Pass `precisionMs` (for example `1000`) to coarsen notifications toward whole-second updates. `get()` stays raw; `elapsed` updates on coarsened notifies. Keep the `elapsed` read in a small component when you still want finer UI.
