...
| `reactiveElapsed` | `boolean` | Default `true`. When `false`, tick notifies do not write the `elapsed` signal; `running` and controls still update. Snapshot `elapsed` on control transitions. |
...
`elapsed` is raw milliseconds delivered at the clock's tick cadence, so anything reading the signal re-renders roughly 60 times a second under `createBrowserClock`. Pass `precisionMs` to coarsen notifies — see [Options](/docs/core/options). Keep the `elapsed` read in the smallest possible component when you still want finer UI.
