...
`elapsed` is raw milliseconds delivered at the clock's tick cadence, so anything reading the signal re-renders roughly 60 times a second under `createBrowserClock`. Pass `precisionMs` (for example `1000`) to coarsen notifications toward whole-second updates. Keep the `elapsed` read in the smallest possible component when you still want finer UI.
