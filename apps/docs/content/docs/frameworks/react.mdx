...
`useStopwatch` owns a stopwatch. `useStore` binds any `Store` (including a shared `Stopwatch`) without taking ownership.
...
Export a module-level or context `Stopwatch`, then bind with `useStore(shared)` in children. Do **not** call `destroy()` from a component that did not create the instance. `useStopwatch` remains the owning entry point.
...
Pass `precisionMs` (for example `1000`) to coarsen notifications toward whole-second updates. `get()` stays raw; `elapsed` updates on coarsened notifies. Keep `elapsed` in the smallest possible component when you still want finer UI.
