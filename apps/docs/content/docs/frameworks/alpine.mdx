...
| `reactiveElapsed` | `boolean` | Default `true`. When `false`, tick notifies do not assign `this.elapsed`; `running` and controls still update. Snapshot `elapsed` on control transitions. |
...
Pass `{ reactiveElapsed: false }` so ticks skip `this.elapsed` assignments while `running` stays reactive. Write live digits from `stopwatch.subscribe` into a DOM node (or use an Alpine `x-effect` that only touches a non-reactive field). See [Vanilla](/docs/frameworks/vanilla).
