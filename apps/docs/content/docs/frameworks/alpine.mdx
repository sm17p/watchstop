...
`elapsed` is raw milliseconds delivered at the clock's tick cadence. Pass `precisionMs` (for example `1000`) to coarsen notifications toward whole-second updates. Keep the `x-text` / binding read in the smallest possible element when you still want finer UI.
