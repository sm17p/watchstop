...
Export `Stopwatch` and `StopwatchOptions` from `@watchstop/core`. Also export `Visibility`, `createMockVisibility`, and `createDocumentVisibility`.
...
- `precisionMs` (optional, finite `> 0`) coarsens **notifications**: ticks still run at clock cadence, but listeners fire only when `floor(elapsed / precisionMs)` changes, plus on forced control notifies (`start` / `stop` / `reset`). `get()` stays **live** raw milliseconds.
- `visibility` defaults to document Page Visibility when `document` exists. Pass `false` to disable. Pass `createMockVisibility()` in tests. While `state() === 'hidden'`, scheduling suspends; on return to `visible`, one catch-up notify runs and the loop resumes if still running.
...
- Otherwise set `running = true`, `startTime = clock.now()`, schedule the tick loop, and **notify** listeners (even when elapsed is still `0`). Adapters sync UI `running` from this notification.
...
While running and not visibility-suspended:

1. Register with the shared driver for `clock` (one `clock.schedule` per clock identity, not per stopwatch).
2. On each shared tick: if still running and not suspended, notify listeners with `get()` subject to `precisionMs` coarsening; the driver re-schedules while any registered stopwatch still wants ticks.

Browser clocks fire once per frame; timer clocks fire once per `intervalMs`. Stopwatch does not care which.

A listener may call `stop` / `reset` / `destroy` during notify — that instance unregisters and is not re-scheduled.
