...
2. `onTick`: if not running or destroyed, return; notify listeners with `get()` subject to `precisionMs` coarsening; if still running and not destroyed, schedule again. A listener may call `stop` / `reset` / `destroy` during notify — do not re-schedule in that case.
