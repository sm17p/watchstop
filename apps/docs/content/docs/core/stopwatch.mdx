...
2. `onTick`: if not running or destroyed, return; else notify listeners with `get()`; then schedule again.

Browser clocks fire once per frame; timer clocks fire once per `intervalMs`. Stopwatch does not care which — it always re-schedules after each callback.
