...
PR/`merge_group` delta scans and the weekly full scan live in [`.github/workflows/osv-scanner.yml`](./.github/workflows/osv-scanner.yml) (SARIF → Security → Code scanning). The release publish path runs the same scanner with `fail-on-vuln` and does not upload SARIF, so it does not register a second Code Scanning config or double-scan on every main push.
